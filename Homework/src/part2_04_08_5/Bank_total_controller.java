...
//					System.out.println("일치하는 회원이 없습니다.");
	...
	void 이체(String loginid) {
		...
		System.out.println("출금하실 계좌 입력 : "); int 출금계좌=scanner.nextInt();
		System.out.println("송금하실 계좌 입력 : "); int 송금계좌=scanner.nextInt();
		for(Bankbook temp:booklist) {
			if(temp!=null && temp.getBankbook_number()==출금계좌) {
				if(temp!=null && temp.getBankbook_number()==송금계좌) {
					System.out.println(" 얼마를 출금 하시겠습니까? ");
					int out_money = scanner.nextInt();
					if(temp.getSave_money()<out_money) {
						System.out.println("잔액이 부족합니다.");
